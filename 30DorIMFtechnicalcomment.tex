...
Figure \ref{fig:IMF} shows the inferred power-law exponent of the IMF.   When using the full data set of \citet{Schneider:2018}, following their assumption that their data set is complete above 15 solar masses, and using their fit to stellar lifetimes, we find an exponent of $\alpharange$ where the quoted value corresponds to the median of
the posterior distribution and the range to the 16th and 84th percentiles (i.e.\
the symmetric 68\% credible interval).  

However, the fit to stellar lifetimes used by \citet{Schneider:2018} continues to decrease at high masses, in contrast to the expectation that stellar lifetimes should asymptote to a fixed minimum duration for high-mass stars set by the ratio of the energy to be released through nuclear fusion to the Eddington luminosity.  We make an independent fit to the lifetimes $\tau{M}$ of non-rotating massive stars as modeled by \citet{Brott:2011}:
\begin{equation}
\log (\tau (M) /\textrm{yr}) = X + XX \log{(M/M_\odot)} + XXX (\log{(M/M_\odot)})^2 + XXXX (\log{(M/M_\odot)})^3.
\end{equation}
%X should already include + log 1.1

of asymptoting to 

The conclusion is somewhat sensitive to
...
checking.  We see no statistical evidence against the claim of \citet{Schneider:2018} that
the survey is complete for $M \geq 15 \, \MSun$, and therefore use this largest available data set (with, correspondingly, the smallest inference uncertainty) for our analysis.

\begin{figure}
%\vspace{-1in}
    		    		\includegraphics[width=\columnwidth]{alpha.pdf}
%\vspace{-1in}
    		\caption{The posterior inferred on the power-law exponent assuming survey completeness for $M \geq 15 \, \MSun$ with $\nu = 100$ degrees of freedom in the Student T likelihood functions (blue curve; see text) and $\nu = 4$ degrees of freedom (green curve).  There is little difference between the blue and green curves, suggesting that the inference on the power-law exponent is insensitive to the heavy-tailed nature of the likelihood function.  In all cases the IMF we infer is consistent with a Salpeter power-law exponent of $-\alpha=-2.35$ \citep{Salpeter:1955}. }\label{fig:IMF}
\end{figure}
...
In conclusion, we find that there is no evidence for a deviation from the Salpeter IMF \citep{Salpeter:1955} in the observations of young massive stars in 30 Doradus once the correct statistical analysis is applied.
